...
func add_button(disp_slave,counter,first_text,first_parametr,second_text,second_parametr,third_text,third_parametr):
	...
	if first_parametr != null and first_text != null:
		label = Label.new()
		label.name = "Skill1"
...
func _on_button_slave_pressed(active_slave,now_pressed):
	...
	if choosen_work == "Ублажать скот":
		...
		Globaldata.money += int(Globaldata.difficult_ratio * active_slave.sexual.xenophilia * 10)
...
func _ready():
	if Globaldata.hero.place_suburb_district[1] == 0:
		Globaldata.hero.place_suburb_district[1] = 1
...
func _on_work_pressed():
	...
	$Dialog.set_text("У меня много работы есть на ферме")
...
func _on_clear_barn_pressed():
	$Actions.set_visible(false)
	$Choose_slave.set_visible(true)
	$Work.set_visible(false)
	$Dialog.set_text("Работа не из пыльных, но запачкать руки придёться")
	choosen_work = "Уборка стойла"
	for i in range(Globaldata.all_slaves.size()):
		update_choose_slave(Globaldata.all_slaves[i],i,"Уборка",Globaldata.all_slaves[i].common.housekeeping,null,Globaldata.all_slaves[i].sexual.pig,null,Globaldata.all_slaves[i].sexual.horse)


func _on_appease_livestock_pressed():
	$Actions.set_visible(false)
	$Choose_slave.set_visible(true)
	$Work.set_visible(false)
	$Dialog.set_text("За такую работу я готов заплатить неплохие деньги, а то разрядки животным не хватает")
	choosen_work = "Ублажать скот"
	for i in range(Globaldata.all_slaves.size()):
		update_choose_slave(Globaldata.all_slaves[i],i,"Кобель",Globaldata.all_slaves[i].sexual.doggy,"Хряк",Globaldata.all_slaves[i].sexual.pig,"Жеребец",Globaldata.all_slaves[i].sexual.horse)
...
func _on_attract_visitors_pressed():
	...
	$Choose_slave.set_visible(true)
	$Work.set_visible(false)
	$Dialog.set_text("Сейчас очень мало рабочих так что если она сможет привлечь ещё рабочих будет хорошо")
	choosen_work = "Привлекать посетителей"
	for i in range(Globaldata.all_slaves.size()):
		update_choose_slave(Globaldata.all_slaves[i],i,"Общение",Globaldata.all_slaves[i].common.etiquette_and_rehetoric,null,Globaldata.all_slaves[i].sexual.pig,null,Globaldata.all_slaves[i].sexual.horse)


func _on_feed_livestock_pressed():
	$Actions.set_visible(false)
	$Choose_slave.set_visible(true)
	$Work.set_visible(false)
	$Dialog.set_text("Сейчас у меня времени сильно не хватает так что я готов заплатить за такую работу но не самые большие деньги")
	choosen_work = "Кормить животных"
	for i in range(Globaldata.all_slaves.size()):
		update_choose_slave(Globaldata.all_slaves[i],i,"Фермерство",Globaldata.all_slaves[i].common.farming,null,Globaldata.all_slaves[i].sexual.pig,null,Globaldata.all_slaves[i].sexual.horse)
